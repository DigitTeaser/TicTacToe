...
public class Game {
    ...
    public String checkGameWinner(char[][] grid) {
        ...
        // Checks whether there is a winner in three columns.
        for (int i = 0; i < 3; i++) {
            // Counter for both X and O in a single column.
            int xCount = 0;
            int oCount = 0;

            // Checks for both X and O in a single column.
            for (int j = 0; j < 3; j++) {
                // If the grid is x, add one to xCount.
                if (grid[i][j] == 'x') {
                    xCount++;
                }
                // If the grid is o, add one to oCount.
                if (grid[i][j] == 'o') {
                    oCount++;
                }
            }

            // Return the winner according to the counter.
            if (xCount == 3) {
                return "X wins";
            }
            if (oCount == 3) {
        ...
        // Checks whether there is a winner in three rows.
        for (int i = 0; i < 3; i++) {
            // Counter for both X and O in a single row.
            int xCount = 0;
            int oCount = 0;

            // Checks for both X and O in a single row.
            for (int j = 0; j < 3; j++) {
                // If the grid is x, add one to xCount.
                if (grid[j][i] == 'x') {
                    xCount++;
        ...
        // Checks whether there is a winner in both diagonals.
        ...
        // If all grids are filled and there is no winner, then return tie.
        int count = 0;
